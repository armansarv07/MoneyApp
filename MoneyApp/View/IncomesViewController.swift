...
class IncomesViewController: UIViewController {
    ...
    @objc private func addIncome() {
        showAlertController(title: "Add Income", message: "Add your income!", mode: .add)
    }
...
extension IncomesViewController {
    func showAlertController(title: String, message: String, mode: AlertControllerMode , style: UIAlertController.Style = .alert, indexPath: IndexPath = IndexPath(index: 0)) {
        ...
        alertController.addTextField { $0.placeholder = "Title"
            if mode == .update {
                $0.text = self.viewModel?.incomes![indexPath.row].title
            }
        }
        alertController.addTextField { $0.placeholder = "Category"
            if mode == .update {
                $0.text = self.viewModel?.incomes![indexPath.row].category
            }
        }
        alertController.addTextField { $0.placeholder = "Amount"; $0.keyboardType = .decimalPad
            if mode == .update {
                if let amount = self.viewModel?.incomes?[indexPath.row].amount {
...
extension IncomesViewController {
    private func setupNavigationItems() {
        ...
        self.navigationItem.title = "Income List"
...
extension IncomesViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        showAlertController(title: "Update", message: "Update income value", mode: .update, style: .alert, indexPath: indexPath)
    }
    
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        let action = UIContextualAction(style: .destructive, title: "Delete Income") { action, view, completionHandler in
...
struct IncomesVCProvider: PreviewProvider {
